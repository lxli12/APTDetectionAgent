...
from apt_detection_agent.controller.frozen_runtime import ActionExecutionEnvelope
...
class FrozenRuntimeCatalog:
    ...
    def _validate_link(
        ...
        if status != AvailabilityStatus.AVAILABLE:
            return
        admission = self.admissions.get(str(admission_id))
        if not isinstance(admission, PIDSAdmissionRecord) or (
            not admission.admitted_for_formal_trajectory
            or admission.pids != pids
            or admission.dataset_or_scenario_id != scenario_id
            or admission.split != split
            or admitted_use not in admission.admitted_uses
        ):
