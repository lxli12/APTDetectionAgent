...
9. Agent-selectable threshold 统一限定为 validation-derived、no-snoop 的 `validation_quantile`、`max_val_loss` 和 `mean_val_loss`；只有 `validation_quantile` 继续选择有限 quantile 值。PIDSMaker 原生方法/值在配置中标注为默认 reference，其中固定 scalar 和 snooped MAGIC threshold 不直接作为 Agent 实际阈值。
...
LLM 先从 `[validation_quantile, max_val_loss, mean_val_loss]` 选择 method。只有 method 为 `validation_quantile` 时才继续选择 `value`，第一版统一为 `[0.90, 0.95, 0.99]`；另外两种 method 没有 value 字段。Harness 从该 checkpoint 的 benign-validation score distribution 解析 `resolved_value`。Quantile `q` 对应的 benign-validation 经验超阈率约为 `1-q`，但受有限样本、重复分数和比较方向影响，不保证恰好相等，更不保证 held-out test 上具有相同 false-positive rate。LLM 不能自由生成 method、quantile 或 `resolved_value`。
...
PIDSMaker 原生 threshold 方法和值必须在对应 PIDS 配置中以默认注释保留，用于说明 baseline provenance；Agent 实际执行统一从上述三种 validation-derived 方法中选择。原生固定 scalar 和使用 held-out test score distribution 的 MAGIC 行为不进入主实验实际 threshold：
...
Agent-compatible MAGIC 使用三种 validation-derived 方法之一，从 benign validation 的 MAGIC anomaly scores 解析 threshold。NODLINK 原生 90% quantile 直接表示为 `method: validation_quantile, value: 0.90`，并标注为 PIDSMaker 默认值；`0.95` 和 `0.99` 是 Agent variants。
...
字段选择空间统一采用 `field: {values: [...]}` 风格。简单字段直接列出有限值；必须耦合的多个字段使用一个结构化 field，其 `values` 中列出完整合法组合。合法组合很多时不在 prompt 中平铺所有完整 configuration，而由树形分支、耦合 value 和条件约束紧凑表达；内部 artifact registry 仍可索引所有已经准备好的叶节点。示例：
  ...
  pids:
      ...
      fields:
        ...
        threshold:
          ...
          value:
            ...
            values: [0.90, 0.95, 0.99]  # PIDSMaker default equivalent: max validation score
    ...
    flash:
      fields:
        threshold:
          method:
            values: [validation_quantile, max_val_loss, mean_val_loss]  # PIDSMaker default: flash (reference only)
          value:
            when: {method: validation_quantile}
            values: [0.90, 0.95, 0.99]  # PIDSMaker default scalar: 0.53 (reference only)
      # upstream_reference_only:
    ...
    threatrace:
      fields:
        threshold:
          method:
            values: [validation_quantile, max_val_loss, mean_val_loss]  # PIDSMaker default: threatrace (reference only)
          value:
            when: {method: validation_quantile}
            values: [0.90, 0.95, 0.99]  # PIDSMaker default scalar: 1.5 (reference only)
      # upstream_reference_only:
      #   threshold.fixed_value:
      #     values: [1.5]
    ...
    magic:
      fields:
        threshold:
          ...
          value:
            when: {method: validation_quantile}
            values: [0.90, 0.95, 0.99]  # no deployable upstream default
      # magic_upstream_snooped:
      #   threshold.method:
