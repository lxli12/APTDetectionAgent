...
| REQ-LABEL-001 | Agent-visible processes cannot access hidden labels or private mappings | 1/3/7 | planned visibility schemas and process boundary | field/permission leakage tests | planned |
| REQ-LABEL-002 | Hidden teacher data is sanitized before student input, rationale, or deployable memory | 1/4/10 | planned sanitizer | privileged-field negative tests | planned |
| REQ-LABEL-003 | Held-out evaluator returns no online step reward; only episode metrics | 7 | planned evaluator protocol | feedback-contract tests | planned |
| REQ-LABEL-004 | Trace routing uses deployment-visible evidence only | 1/2/5 | planned trace schema | hidden-annotation rejection tests | planned |
| REQ-WINDOW-001 | Windows record origin, timezone, size, and `[start,end)` bounds | 1/3 | planned Window schema | boundary tests | planned |
...
| REQ-WINDOW-003 | Current window uses committed fast-path configuration | 1/5 | planned Case/config schemas | state-transition tests | planned |
...
| REQ-CONFIG-001 | Persistent reconfiguration takes effect at the next window | 1/5 | planned effective-window fields | same-window rejection tests | planned |
| REQ-CONFIG-002 | Held-out selects only frozen ApprovedConfig and thresholds | 1/2/5 | planned catalog schemas/tools | catalog rejection tests | planned |
| REQ-CONFIG-003 | Threshold records carry method, split, checkpoint, metric, time, and commit provenance | 1 | planned Threshold schema | missing-field tests | planned |
...
| REQ-PIDS-002 | Unavailable PIDS remain registered with explicit reasons | 1/2 | planned availability schema | unavailable retention test | planned |
| REQ-PIDS-003 | ORTHRUS fixed/non-snooped are variants, not new methods | 1/2 | planned normalized identity | inventory tests | planned |
...
| REQ-TOOL-001 | LLM emits typed requests and never constructs shell commands | 1/2/5 | planned ToolRequest/validator | metacharacter/unknown-field tests | planned |
...
| REQ-TOOL-003 | Every tool call records validated args, config/checkpoint, command manifest, timing, output, and artifacts | 1/2 | planned ToolResult/RunManifest | provenance completeness tests | planned |
...
| REQ-MEMORY-001 | Working, episode memory, and case state reset at split/scenario boundaries | 1/4 | planned memory namespace | cross-boundary tests | planned |
...
| REQ-MEMORY-003 | Frozen deployable static LTM may cross splits only as a sanitized training artifact | 1/4 | planned LTM manifest | sanitizer/signature tests | planned |
...
| REQ-EVAL-001 | Main evaluation uses campaign coverage and unique malicious-node TP/FP/FN | 1/7 | planned manifests/metrics | hand-computed fixtures | planned |
| REQ-EVAL-002 | Node-window, edge, chain, phase, provenance, latency, GPU, and tool-call metrics have separate denominators | 1/7 | planned EvaluationRecord | denominator tests | planned |
| REQ-EVAL-003 | P@C=100%, MCC, and ADP definitions are versioned | 1/7 | planned metric definitions | edge-case fixtures | planned |
| REQ-EVAL-004 | Campaign identity comes from a versioned manifest, not a ground-truth filename | 1/7 | planned CampaignManifest | manifest validation tests | planned |
...
| REQ-ARTIFACT-001 | Every artifact records source config, checkpoint hash, code commit, and producing stage | 1/2 | planned manifests | provenance tests | planned |
| REQ-ARTIFACT-002 | Missing checkpoints produce unavailable status, never fabricated artifacts | 1/2/8 | planned registry/executor | missing-checkpoint tests | planned |
...
| REQ-ENV-004 | Controller environment stays minimal and excludes PyTorch/PyG/vLLM | 1 | planned dependency ADR | dependency audit | planned |
...
| REQ-SFT-002 | Hidden teacher inputs and student-visible outputs use separate schemas | 1/10 | planned schemas | boundary tests | planned |
