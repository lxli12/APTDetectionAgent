...
| REQ-CAUSAL-004 | Transductive runs are labeled compatibility baselines and excluded from main results | 1/2/3/7/9 | config/result schemas; fitted-state causal-main gate; synthetic run evidence class | transductive rejection and false-formal-claim tests; real comparison deferred | partial |
...
| REQ-TOOL-005 | Timeout, nonzero exit, malformed output, and missing artifact fail closed | 2/5/8 | `pidsmaker/adapter.py`; `tooling/memory_tools.py`; `scripts/pidsmaker_stage_runner.py` | negative tests; preserved typed failures `structured-adapter-20260714-002/004/005`; real typed parser success `structured-adapter-20260714-006` | implemented |
...
| REQ-ARTIFACT-003 | Raw PIDSMaker artifacts are mapped and validated before becoming Agent contracts | 2/8 | artifact map; compatibility report; stage runner; VELOX score parser | missing/privileged-artifact tests and real VELOX parser run `structured-adapter-20260714-006`; parsers for unavailable PIDS remain deferred | partial |
