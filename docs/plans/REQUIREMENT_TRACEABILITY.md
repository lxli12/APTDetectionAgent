...
| REQ-TOOL-003 | Every tool call records validated args, config/checkpoint, command manifest, timing, output, and artifacts | 1/2/5/8 | `ToolResult`; `CommandManifest`; Phase 2 adapter; memory/case/report audit JSONL; safe stage summary | structured-tool audit tests and synthetic runtime artifacts; real PIDS trace deferred | partial |
...
| REQ-TOOL-005 | Timeout, nonzero exit, malformed output, and missing artifact fail closed | 2/5/8 | `pidsmaker/adapter.py`; `tooling/memory_tools.py`; `scripts/pidsmaker_stage_runner.py` | adapter failures; structured-tool identity/argument/overwrite failures; stage path/override/env/overwrite negative tests; real typed parser deferred | partial |
...
| REQ-MEMORY-007 | Retrieval budget and candidate cap are validation-tuned engineering defaults | 4/9 | `RetrievalPolicy` with unvalidated status; end-to-end retrieval | cap/budget/false-optimal-claim and multi-window retrieval tests; sensitivity experiment deferred | partial |
